...
class DefectSelect extends Component
{
    ...
    {
        // 取得最新生效的缺陷日期
        $latestDefect = Defect::whereDate('effective_date', '<=', today())->orderBy('effective_date', 'desc')->first()->effective_date;
        // 轉換成 Carbon 格式
        $latestDefect = Carbon::create($latestDefect);

        // 取得所有不重複的群組
        $distinctGroups = Defect::getDistinctGroups($latestDefect);
        // 取得所有不重複的群組下的標題
        $distinByGroupsTitles = Defect::getDistinctTitlesByGroup($this->group, $latestDefect);
        // 取得所有不重複的群組下的標題下的描述
