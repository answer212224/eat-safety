                ...
                <div class="widget-content widget-content-area">
                    <div class="bs-stepper stepper-form-one">
                        ...
                        @if ($task->category == '清潔檢查')
                            <form action="{{ route('task-clear-defect-store', ['task' => $task->id]) }}" method="post"
                                enctype="multipart/form-data" id="defect_form">
                            @else
                        ...
                        <div class="bs-stepper-content">
                            ...
                            <div id="two" class="content" role="tabpanel">
                                ...
                                <div class="button-action mt-5 text-center">
                                    <a class="btn btn-secondary btn-prev me-3" onclick="stepper.previous()">上一步</a>
                                    <a class="btn btn-secondary btn-nxt" onclick="stepper.next()">下一步</a>
                                </div>
                            </div>
        ...
        <script>
            ...
            stepperEl.addEventListener('show.bs-stepper', function(event) {
                if (event.detail.indexStep == 1) {
                    ...
                    // 假如不是圖片
                    if (window.multifiles.getFiles()[0].fileType != 'image/png' && window.multifiles.getFiles()[0]
                        .fileType != 'image/jpeg' && window.multifiles.getFiles()[0].fileType != 'image/jpg') {
                        event.preventDefault();
                        alert('請上傳正確檔案格式');
                    }
                    // 假如還再上傳中
