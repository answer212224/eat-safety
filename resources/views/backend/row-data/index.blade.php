                        ...
                        <tbody>
                            @foreach ($tableBodys as $body)
                                <tr>
                                    ...
                                    <td>{{ $body['restaurant_name'] }}</td>
                                    <td>{{ $body['restaurant_code'] }}</td>
                                    <td>{{ $body['back_score'] }}</td>
                                    <td>{{ $body['back_manager'] }}</td>
                                    <td>{{ $body['front_score'] }}</td>
                                    <td>{{ $body['front_manager'] }}</td>
                                    <td>{{ $body['auditor'] }}</td>
                                    <td>{{ $body['auditor_2'] }}</td>
                                    <td>{{ $body['auditor_3'] }}</td>
                                    @foreach ($body['distinctGroupsCount'] as $item)
                                        <td>{{ $item }}</td>
                                    @endforeach
                                    @foreach ($body['backTaskNot5S'] as $item)
                                        {{-- 假設item是array顯示內容 --}}
                                        @if (is_array($item))
                                            {{-- 提示區站名稱 --}}
                                        ...
                                        @endif
                                    @endforeach
                                    <td class="bs-tooltip" title="{{ $body['frontTaskNot5S']['area'] }}">
                                        {{ $body['frontTaskNot5S']['count'] }}
                                    </td>
                                    @foreach ($body['backTask5S'] as $item)
                                        @if (is_array($item))
                                            {{-- 提示區站名稱 --}}
                                            <td class="bs-tooltip" title="{{ $item['area'] }}">
                                                {{ $item['count'] }}
